...
-- ║   CLI      : mysql -u root -p < milestone3.sql                  ║
-- ╚══════════════════════════════════════════════════════════════════╝
--
-- ┌─────────────────────────────────────────────────────────────────┐
-- │ CONTENTS                                                        │
-- │  SECTION 1  ─ Database creation              (run standalone)  │
-- │  SECTION 2  ─ Table definitions / DDL        (run standalone)  │
-- │  SECTION 3  ─ Indexes                        (run standalone)  │
-- │  SECTION 4  ─ Views / derived attributes     (run standalone)  │
-- │  SECTION 5  ─ Stored functions               (run standalone)  │
-- │  SECTION 6  ─ Stored procedures              (run standalone)  │
-- │  SECTION 7  ─ Disjoint constraint triggers   (run standalone)  │
-- │  SECTION 8  ─ Sample data (DML)              (run standalone)  │
-- │  SECTION 9  ─ Constraint validation tests    (run standalone)  │
-- │  SECTION 10 ─ Business demonstration queries (run standalone)  │
-- └─────────────────────────────────────────────────────────────────┘


-- ══════════════════════════════════════════════════════════════════
-- SECTION 1: DATABASE CREATION
-- Run this section first, once only.
-- ══════════════════════════════════════════════════════════════════
...
CREATE DATABASE agri_services_db
    ...
    COMMENT     'Agricultural Services DB — Ministry of Agriculture Uganda';
...
--   smallholder_farmer→ Subtype L2 of farmer  (disjoint)
--   commercial_farmer → Subtype L2 of farmer  (disjoint)
...
--   robusta_farm      → Subtype of farm (overlapping)
--   arabica_farm      → Subtype of farm (overlapping)
--   mixed_farm        → Subtype of farm (overlapping)
...
--   complaint_feedback→ Weak entity → farmer
-- ══════════════════════════════════════════════════════════════════
...
-- ────────────────────────────────────────────────────────────────
-- 2.26  COMPLAINT_FEEDBACK  (Weak entity — depends on FARMER)
-- ────────────────────────────────────────────────────────────────
CREATE TABLE complaint_feedback (
    complaint_id      INT UNSIGNED NOT NULL AUTO_INCREMENT,
    farmer_id         INT UNSIGNED NOT NULL,
    date_raised       DATE         NOT NULL,
    category    ENUM('Input Quality','Late Delivery','Advisory',
                     'Extension Worker','General','Other') NOT NULL,
    description       TEXT         NOT NULL,
    resolution_status ENUM('Open','In Progress','Resolved','Dismissed')
                      NOT NULL DEFAULT 'Open',
    resolved_date     DATE         NULL,
    CONSTRAINT pk_complaint_feedback PRIMARY KEY (complaint_id),
    CONSTRAINT fk_cf_farmer
        FOREIGN KEY (farmer_id) REFERENCES farmer(farmer_id)
        ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT chk_cf_resolved
        CHECK (resolved_date IS NULL OR resolved_date >= date_raised)
) ENGINE = InnoDB COMMENT = 'Weak entity — farmer complaints and service feedback';
...
CREATE INDEX idx_complaint_status    ON complaint_feedback(resolution_status);


-- ══════════════════════════════════════════════════════════════════
-- SECTION 4: VIEWS  (Derived Attributes & Reporting)
-- Run each CREATE VIEW statement individually if needed.
-- ══════════════════════════════════════════════════════════════════
...
-- Run each function block individually (include the DELIMITER lines).
-- ══════════════════════════════════════════════════════════════════
...
-- Each procedure is a self-contained block with transaction handling.
...
-- These triggers enforce the EERD disjoint specialisation rules.
-- Run each DELIMITER block individually.
-- ══════════════════════════════════════════════════════════════════
...
-- Insert statements for all 26 tables.
...
INSERT INTO person
    (national_id, full_name, date_of_birth, gender,
     phone_number, email, district, village_lc1)
VALUES
('CM901001A','Nakato Aisha',       '1985-03-12','Female','0772100001','nakato@gmail.com', 'Mukono', 'Namanve'),
